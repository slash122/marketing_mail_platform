...
from src.services.accounts import create_account, get_account_senders, select_all_accounts, select_account_by_id, add_senders_to_account
...
@router.get("/accounts", dependencies=[Depends(check_if_admin)], response_model=list[AccountResponse], status_code=status.HTTP_200_OK)
...
@router.post("/accounts/register", dependencies=[Depends(check_if_admin)], response_model=AccountResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/accounts/{account_id}", dependencies=[Depends(check_if_admin)], response_model=AccountResponse, status_code=status.HTTP_200_OK)
...
@router.get("/accounts/{account_id}/senders", dependencies=[Depends(check_if_admin)], response_model=list[SenderResponse], status_code=status.HTTP_200_OK)
